...
| 阶段 | 做什么 | 产出 | 预计时间 |
|------|--------|------|----------|
| 0 | 读懂官方资料 | [阶段0实现报告.md](../阶段0/阶段0实现报告.md) | 0.5 天 |
| 1 | 搭环境 | [阶段1实现报告.md](../阶段1/阶段1实现报告.md) | 0.5 天 |
...
**完成后填写：** [阶段0实现报告.md](../阶段0/阶段0实现报告.md)（故事版）  
**快速认可：** [阶段0快速验收清单.md](../阶段0/阶段0快速验收清单.md)（5～10 分钟勾表）  
**逐步验收（推荐）：** [阶段0验收操作手册.md](../阶段0/阶段0验收操作手册.md)（30～40 分钟，含截图与功能对照）  
...
**完成后填写：** [阶段1实现报告.md](../阶段1/阶段1实现报告.md)  
**快速勾表：** [阶段1快速验收清单.md](../阶段1/阶段1快速验收清单.md)

### 要做什么

**一键（推荐）：** 根目录双击 `setup阶段1环境.bat`
...
1. 配置 `project/.env`（`AGNES_API_KEY` 阶段 0 已有；`DEEPSEEK_API_KEY` 阶段 4 前补全）  
2. 从 Fork 复制 `implementation.py` → `project/`（路径见 [IMPLEMENTATION.md](../../project/IMPLEMENTATION.md)）  

官方执行器路径（agent-engineering 仓库内）：

`quanttide-devops/loops/devops-code/implementation.py`

### 验收标准

- [ ] `python scripts/verify_stage1.py` 通过（E09 复制执行器后可全绿）  
- [ ] `python -c "import yaml, langgraph"` 无报错（在 `.venv` 内）  
- [ ] `python check.py --help` 能运行  
- [ ] `project/.env` 已配置  
...
└── project/
    ...
    └── trials/case-01/
        ├── input/journal-official-full.md
        ├── prompts/
        └── output/               ← 你跑出来后才有
```
